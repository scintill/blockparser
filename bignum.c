/*	bignum.c
    based on http://www.cs.sunysb.edu/~skiena/392/programs/bignum.c
...
#define MAXDIGITS BIGNUM_MAXDIGITS
...
void byte_arr_to_bignum(unsigned char *s, int bytes, bignum *n) {
    bignum n256;
    int_to_bignum(256, &n256);

    bignum n2;

    for (int_to_bignum(0, n), s += bytes-1; bytes > 0; bytes--, s--) {
        bignum t;
        int_to_bignum(*s, &t);

        multiply_bignum(n, &n256, &n2);
        add_bignum(&n2, &t, n);
    }
...
int bignum_to_str(bignum *n, char *s, int l) {
    int j = 0;
#define OUTC(c) if(j > l) return j; s[j++] = c;
	int i;

	if (n->signbit == MINUS) { OUTC('-'); OUTC(' '); }
	for (i=n->lastdigit; i>=0; i--) {
		OUTC('0'+ n->digits[i]);
    }
